 ...
 *
 * If an order with the same razorpayOrderId + status=paid already exists,
 * it returns early with `alreadyExisted: true`.
 *
 * Both verify-payment (browser flow) and the Razorpay webhook call this
...
export async function createOrder(
  ...
  if (!quickCheck.empty) {
    console.log('[createOrder] ⚡ Fast-path: order already exists for', razorpayOrderId);
